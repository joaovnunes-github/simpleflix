...
@DisplayName("S3MediaPreviewRepository Test")
class S3MediaPreviewRepositoryTest {
    @Mock
    private S3Client s3Client;

    @InjectMocks
    private S3MediaPreviewRepository s3MediaPreviewRepository;
        ...
        when(s3Client.getObject(ArgumentMatchers.any(GetObjectRequest.class)))
                .then(
                        invocation -> {
                            ...
                            assertEquals(getObjectRequest.key(), "preview/media/" + uuid);

                            return responseInputStream;
                        });

        InputStream result = s3MediaPreviewRepository.findBy(uuid);
