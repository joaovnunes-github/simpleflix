...
@DisplayName("S3ImagePreviewRepository Test")
class S3ImagePreviewRepositoryTest {
    @Mock
    private S3Client s3Client;

    @InjectMocks
    private S3ImagePreviewRepository s3ImagePreviewRepository;
        ...
        when(s3Client.getObject(ArgumentMatchers.any(GetObjectRequest.class)))
                .then(
                        invocation -> {
                            ...
                            assertEquals(getObjectRequest.key(), "preview/image/" + uuid);

                            return responseInputStream;
                        });

        InputStream result = s3ImagePreviewRepository.findBy(uuid);
        assertEquals(result, responseInputStream);
    }
}
